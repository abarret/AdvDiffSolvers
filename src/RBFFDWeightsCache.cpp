...
#endif
    auto it = std::find(d_base_pt_vec[patch].begin(), d_base_pt_vec[patch].end(), pt);
    size_t l = std::distance(it, d_base_pt_vec[patch].end());
    return d_pt_weight_vec[patch][l];
}
...
RBFFDWeightsCache::getRBFFDPoints(SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM>> patch, const UPoint& pt)
{
    ...
    auto it = std::find(d_base_pt_vec[patch].begin(), d_base_pt_vec[patch].end(), pt);
    size_t l = std::distance(it, d_base_pt_vec[patch].end());
    return d_pair_pt_vec[patch][l];
}

bool
RBFFDWeightsCache::isRBFFDBasePoint(SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM>> patch, const UPoint& pt)
{
    return std::find(d_base_pt_vec[patch].begin(), d_base_pt_vec[patch].end(), pt) != d_base_pt_vec[patch].end();
}
...
RBFFDWeightsCache::printPtMap(std::ostream& os)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++patch_num)
    {
        ...
        os << "There are " << d_base_pt_vec[patch].size() << " key-value pairs present\n";
        for (size_t i = 0; i < d_base_pt_vec[patch].size(); ++i)
        {
            const UPoint& pt = d_base_pt_vec[patch][i];
            const std::vector<UPoint>& pt_vec = d_pair_pt_vec[patch][i];
            os << "  Looking at point:\n" << pt << "\n";
            os << "  Has points: \n";
...
RBFFDWeightsCache::sortLagDOFsToCells()
{
    ...
    for (auto it = mesh.nodes_begin(); it != it_end; ++it)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            if (box.contains(idx)) d_idx_node_vec[patch].push_back(node);
            Box<NDIM> ghost_box = box;
            ghost_box.grow(IntVector<NDIM>(s_num_ghost_cells));
            if (ghost_box.contains(idx)) d_idx_node_ghost_vec[patch].push_back(node);
        }
    }
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++)
    {
        ...
        for (const auto& node : d_idx_node_ghost_vec[patch])
        {
            VectorNd node_pt;
            for (unsigned int d = 0; d < NDIM; ++d)
            {
        ...
        for (CellIterator<NDIM> ci(patch->getBox()); ci; ci++)
        {
            ...
            if (ls < -d_eps && std::abs(ls) < d_dist_to_bdry)
            {
                ...
                d_base_pt_vec[patch].push_back(UPoint(patch, idx));
                d_pair_pt_vec[patch].push_back({});
#if (1)
                // Use KNN search.
                ...
                for (const auto& idx_in_pts : idx_vec) d_pair_pt_vec[patch][i].push_back(pts[idx_in_pts]);
                ++i;
            }
        }
        // Now do Lagrangian points
        for (const auto& node : d_idx_node_vec[patch])
        {
            VectorNd node_pt;
            for (unsigned int d = 0; d < NDIM; ++d)
            {
            ...
            std::vector<int> idx_vec;
            std::vector<double> distance_vec;
            d_base_pt_vec[patch].push_back(UPoint(patch, node));
            d_pair_pt_vec[patch].push_back({});
            tree.knnSearch(UPoint(node_pt, node), interp_size, idx_vec, distance_vec);
            // Add these points to the vector
            for (const auto& idx_in_pts : idx_vec) d_pair_pt_vec[patch][i].push_back(pts[idx_in_pts]);
            ++i;
        }
...
RBFFDWeightsCache::findRBFFDWeights()
{
    ...
    {
        Pointer<Patch<NDIM>> patch = level->getPatch(p());
        if (d_base_pt_vec[patch].size() == 0) continue;
        ...
        d_pt_weight_vec[patch].resize(d_base_pt_vec[patch].size());
        for (size_t idx = 0; idx < d_base_pt_vec[patch].size(); ++idx)
        {
            const UPoint& pt = d_base_pt_vec[patch][idx];
            const std::vector<UPoint>& pt_vec = d_pair_pt_vec[patch][idx];
            ...
            for (int i = 0; i < interp_size; ++i) d_pt_weight_vec[patch][idx].push_back(weights(i));
        }
    }
}
//////////////////////////////////////////////////////////////////////////////
