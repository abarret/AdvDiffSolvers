...
/////////////////////////////// PUBLIC ///////////////////////////////////////
FEToHierarchyMapping::FEToHierarchyMapping(std::string object_name,
                                           FESystemManager* fe_sys_manager,
                                           const Pointer<Database>& input_db,
                                           const int max_levels,
                                           IntVector<NDIM> ghost_width)
    : d_fe_sys_manager(fe_sys_manager),
      d_level_lookup(max_levels - 1,
                     collect_subdomain_ids(fe_sys_manager->getFEData()->getEquationSystems()->get_mesh()),
      ...
      d_ghost_width(std::move(ghost_width))
{
    ...
    IBTK_DO_ONCE(t_reinit_element_mappings =
                     TimerManager::getManager()->getTimer("ADS::FEToHierarchyMapping::reinitElementMappings()");
                 t_build_ghosted_solution_vector =
                     TimerManager::getManager()->getTimer("ADS::FEToHierarchyMapping::buildGhostedSolutionVector()");
                 t_build_ghosted_vector =
                     TimerManager::getManager()->getTimer("ADS::FEToHierarchyMapping::buildGhostedVector()");)
    return;
} // FEToHierarchyMapping

const FESystemManager&
FEToHierarchyMapping::getFESystemManager() const
{
    return *d_fe_sys_manager;
}

FESystemManager&
FEToHierarchyMapping::getFESystemManager()
{
    return *d_fe_sys_manager;
}

const std::string&
FEToHierarchyMapping::getCoordsSystemName() const
{
    return d_fe_sys_manager->getCoordsSystemName();
}

void
FEToHierarchyMapping::setPatchHierarchy(Pointer<PatchHierarchy<NDIM>> hierarchy)
{
    // Reset the hierarchy.
...
FEToHierarchyMapping::getPatchHierarchy() const
{
    return d_hierarchy;
} // getPatchHierarchy

int
FEToHierarchyMapping::getCoarsestPatchLevelNumber() const
{
...
int
FEToHierarchyMapping::getFinestPatchLevelNumber() const
{
...
FEToHierarchyMapping::getGhostCellWidth() const
{
    return d_ghost_width;
} // getGhostCellWidth

const std::vector<std::vector<Elem*>>&
FEToHierarchyMapping::getActivePatchElementMap(int ln) const
{
...
FEToHierarchyMapping::getActivePatchNodeMap(int ln) const
{
    if (ln == IBTK::invalid_level_number)
...
void
FEToHierarchyMapping::reinitElementMappings(const IntVector<NDIM>& ghost_width)
{
    ...
    // its content is no longer relevant. Note that this may be inefficient if fe_sys_manager is being used with
    // multiple hierarchy mapping objects.
    d_fe_sys_manager->clearPatchHierarchyDependentData();
    ...
    const std::string& coords_sys_name = d_fe_sys_manager->getCoordsSystemName();
    const std::shared_ptr<FEData> fe_data = d_fe_sys_manager->getFEData();
    ...
    for (int ln = 0; ln <= d_hierarchy->getFinestLevelNumber(); ++ln)
    {
        collectActivePatchElements(
            d_active_patch_elem_map[ln], fe_data, coords_sys_name, d_hierarchy->getPatchLevel(ln), d_ghost_width);
        collectActivePatchNodes(d_active_patch_node_map[ln], d_active_patch_elem_map[ln], fe_data);
    }
    ...
    // are on the interior of some patch (or outside the domain) at the moment.
    {
        ...
        const MeshBase& mesh = d_fe_sys_manager->getEquationSystems()->get_mesh();
        ...
        const DofMap& X_dof_map = fe_data->getEquationSystems()->get_system(coords_sys_name).get_dof_map();
        ...
        if (rank == 0)
        {
            const std::string message =
                ...
                "of FEToHierarchyMapping for more information.";
...
FEToHierarchyMapping::getSolutionVector(const std::string& system_name) const
{
    return d_fe_sys_manager->getEquationSystems()->get_system(system_name).solution.get();
} // getSolutionVector

NumericVector<double>*
FEToHierarchyMapping::buildGhostedSolutionVector(const std::string& system_name, const bool localize_data)
{
    ...
    if (!d_system_ghost_vec.count(system_name))
    {
        if (d_enable_logging)
        {
            plog << "FEToHierarchyMapping::buildGhostedSolutionVector(): building ghosted solution vector for system: "
                 << system_name << "\n";
        }
...
FEToHierarchyMapping::buildIBGhostedVector(const std::string& system_name)
{
    ...
    // Sanity check:
    const System& system = d_fe_sys_manager->getEquationSystems()->get_system(system_name);
...
NumericVector<double>*
FEToHierarchyMapping::getCoordsVector() const
{
    return getSolutionVector(d_fe_sys_manager->getCoordsSystemName());
} // getCoordsVector

NumericVector<double>*
FEToHierarchyMapping::buildGhostedCoordsVector(const bool localize_data)
{
    return buildGhostedSolutionVector(d_fe_sys_manager->getCoordsSystemName(), localize_data);
} // buildGhostedCoordsVector

NumericVector<double>*
FEToHierarchyMapping::buildDiagonalL2MassMatrix(const std::string& system_name)
{
    return d_fe_sys_manager->buildDiagonalL2MassMatrix(system_name);
} // buildDiagonalL2MassMatrix

PetscVector<double>*
FEToHierarchyMapping::buildIBGhostedDiagonalL2MassMatrix(const std::string& system_name)
{
    if (!d_L2_proj_matrix_diag_ghost.count(system_name))
    {
        std::unique_ptr<PetscVector<double>> M_vec = buildIBGhostedVector(system_name);
        *M_vec = *d_fe_sys_manager->buildDiagonalL2MassMatrix(system_name);
        M_vec->close();
...
void
FEToHierarchyMapping::setLoggingEnabled(bool enable_logging)
{
...
bool
FEToHierarchyMapping::getLoggingEnabled() const
{
    return d_enable_logging;
} // getLoggingEnabled

int
FEToHierarchyMapping::getPatchLevel(const Elem* elem) const
{
    return d_level_lookup[elem->subdomain_id()];
}

void
FEToHierarchyMapping::reinitializeIBGhostedDOFs(const std::string& system_name)
{
    ...
    if (!d_active_patch_ghost_dofs.count(system_name))
    {
        ...
        collectGhostDOFIndices(ib_ghost_dofs, d_active_nodes, system_name, d_fe_sys_manager->getFEData());
