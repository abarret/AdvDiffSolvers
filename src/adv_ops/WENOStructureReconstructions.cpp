...
namespace ADS
{
WENOStructureReconstructions::WENOStructureReconstructions(std::string object_name,
...
WENOStructureReconstructions::~WENOStructureReconstructions()
{
...
void
WENOStructureReconstructions::applyReconstruction(const int Q_idx, const int N_idx, const int path_idx)
{
    int coarsest_ln = 0;
...
void
WENOStructureReconstructions::allocateOperatorState(Pointer<PatchHierarchy<NDIM>> hierarchy,
                                                    double current_time,
                                                    double new_time)
{
...
void
WENOStructureReconstructions::deallocateOperatorState()
{
...
void
WENOStructureReconstructions::setCutCellMapping(Pointer<CutCellVolumeMeshMapping> cut_cell_mapping)
{
    d_cut_cell_mapping = std::move(cut_cell_mapping);
}

void
WENOStructureReconstructions::setQSystemName(std::string Q_sys_name)
{
    d_Q_sys_name = std::move(Q_sys_name);
}

void
WENOStructureReconstructions::applyReconstructionLS(const int Q_idx, const int N_idx, const int path_idx)
{
    int coarsest_ln = 0;
    int finest_ln = d_hierarchy->getFinestLevelNumber();
#ifndef NDEBUG
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++patch_num)
        {
            ...
            Pointer<CellData<NDIM, double>> indicator_data = patch->getPatchData(d_indicator_idx);
            ...
            for (CellIterator<NDIM> ci(box); ci; ci++)
            {
                ...
                // Check that the WENO stencil is contained inside the level set.
                if (Reconstruct::within_weno5_stencil(idx, *ls_data) && ADS::node_to_cell(idx, *ls_data) < 0.0)
                {
                    VectorNd x_loc;
                    for (int d = 0; d < NDIM; ++d) x_loc[d] = (*xstar_data)(idx, d);
                    (*Q_new_data)(idx) = Reconstruct::weno5(*Q_cur_data, idx, x_loc);
                    (*indicator_data)(idx) = 1;
                }
                else if (ADS::node_to_cell(idx, *ls_new_data) < 0.0)
                {
                    IBTK::VectorNd x_loc;
                    for (int d = 0; d < NDIM; ++d) x_loc(d) = (*xstar_data)(idx, d);
                        ...
                        (*Q_new_data)(idx) =
                            Reconstruct::radialBasisFunctionReconstruction(x_loc,
                                                                           ...
                                                                           *ls_data,
                                                                           ...
                                                                           d_rbf_order,
                                                                           d_rbf_stencil_size);
                        (*indicator_data)(idx) = 2;
                    }
                }
                else
                {
                    (*Q_new_data)(idx) = 0.0;
                    (*indicator_data)(idx) = 0;
                }
            }
        }
    }
