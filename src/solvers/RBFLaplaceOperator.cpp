...
unsigned int RBFLaplaceOperator::s_num_ghost_cells = 3;
/////////////////////////////// PUBLIC ///////////////////////////////////////
...
void
RBFLaplaceOperator::printPtMap(std::ostream& os)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++patch_num)
    {
        ...
        {
            const UPoint2& pt = d_base_pt_vec[patch_num][i];
            const std::vector<UPoint2>& pt_vec = d_pair_pt_vec[patch_num][i];
            os << "  Looking at point:\n" << pt << "\n";
            os << "  Has points: \n";
...
RBFLaplaceOperator::applyToLagDOFs(const int x_idx, const int y_idx)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++patch_num)
    {
        ...
        {
            const UPoint2& pt = d_base_pt_vec[patch_num][idx];
            if (pt.isNode())
            {
                // We've already set boundary conditions
                continue;
            }
            const std::vector<UPoint2>& pt_vec = d_pair_pt_vec[patch_num][idx];
...
void
RBFLaplaceOperator::sortLagDOFsToCells()
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++patch_num)
    {
        ...
        std::vector<UPoint2> pts;
        for (CellIterator<NDIM> ci(ls_data->getGhostBox()); ci; ci++)
        {
            const CellIndex<NDIM>& idx = ci();
            if ((*ls_data)(idx) < -d_eps) pts.push_back(UPoint2(patch, idx));
        }
        // Now add in the points in d_idx_node_vec
        for (const auto& node : d_idx_node_ghost_vec[patch_num]) pts.push_back(UPoint2(patch, node));

        // Now create KD tree
        tree::KDTree<UPoint2> tree(pts);
        ...
        for (CellIterator<NDIM> ci(patch->getBox()); ci; ci++)
        {
            ...
            if (ls < -d_eps && std::abs(ls) < d_dist_to_bdry)
            {
                ...
                d_base_pt_vec[patch_num].push_back(UPoint2(patch, idx));
                d_pair_pt_vec[patch_num].push_back({});
#if (1)
                // Use KNN search.
                tree.knnSearch(UPoint2(patch, idx), interp_size, idx_vec, distance_vec);
#else
                // Use a bounding box search
                VectorNd bbox;
                bbox(0) = 2.0 * dx[0];
                bbox(1) = 2.0 * dx[1];
                tree.cuboid_query(UPoint2(patch, idx), bbox, idx_vec, distance_vec);
#endif
        ...
        for (const auto& node : d_idx_node_vec[patch_num])
        {
            std::vector<int> idx_vec;
            std::vector<double> distance_vec;
            d_base_pt_vec[patch_num].push_back(UPoint2(patch, node));
            d_pair_pt_vec[patch_num].push_back({});
            tree.knnSearch(UPoint2(patch, node), interp_size, idx_vec, distance_vec);
            // Add these points to the vector
            for (const auto& idx_in_pts : idx_vec) d_pair_pt_vec[patch_num][i].push_back(pts[idx_in_pts]);
            ++i;
        }
...
void
RBFLaplaceOperator::findRBFFDWeights()
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++patch_num)
    {
        ...
        for (size_t idx = 0; idx < d_base_pt_vec[patch_num].size(); ++idx)
        {
            ...
            for (int i = 0; i < interp_size; ++i) d_pt_weight_vec[patch_num][idx].push_back(weights(i));
        }
    }
}

double
RBFLaplaceOperator::getSolVal(const UPoint2& pt, const CellData<NDIM, double>& Q_data, Vec& vec) const
{
    double val = 0.0;
    if (pt.isNode())
    {
...
void
RBFLaplaceOperator::setSolVal(const double val, const UPoint2& pt, CellData<NDIM, double>& Q_data, Vec& vec) const
{
    if (pt.isNode())
    {
        // We're on a node. Need to grab value from augmented vec
