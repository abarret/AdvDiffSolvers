...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &                        0:1)
        ...
        integer un_gcw
        double precision un_0((ilower0-un_gcw):(iupper1+un_gcw),
     &                        (ilower1-un_gcw):(iupper1+un_gcw+1))
        double precision un_1((ilower0-un_gcw):(iupper1+un_gcw+1),
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &                        0:1)
        ...
        integer u_gcw
        double precision u_0((ilower0-u_gcw):(iupper1+u_gcw),
     &                       (ilower1-u_gcw):(iupper1+u_gcw+1))
        double precision u_1((ilower0-u_gcw):(iupper1+u_gcw+1),
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &      x0, x1, u0_ret, u1_ret)
        ...
        integer u_gcw
        double precision u0((ilower0-u_gcw):(iupper1+u_gcw),
     &                      (ilower1-u_gcw):(iupper1+u_gcw+1))
        double precision u1((ilower0-u_gcw):(iupper1+u_gcw+1),
     &                      (ilower1-u_gcw):(iupper1+u_gcw))
     ...
     &           *(x0 - (i0 + 0.5d0))*(x1 - (i1 + 0.5d0))

        if(x1 .lt. (i1 + 0.5d0)) then
          i_ll = i0
          j_ll = i1
          i_ul = i0+1
          j_ul = i1
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &       .and. ls_ul .gt. 0.d0 .and. ls_uu .gt. 0.d0)) then
          ...
          if (ls_ll * ls_lu .lt. 0.d0) then
            x_bounds(num) = i0
            y_bounds(num) = i1 - ls_ll / (ls_lu - ls_ll)
            num = num + 1
          endif
          if (ls_lu .lt. 0.d0) then
            x_bounds(num) = i0
            y_bounds(num) = i1 + 1.d0
            num = num + 1
          endif
          if (ls_lu * ls_uu .lt. 0.d0) then
            x_bounds(num) = i0 - ls_lu / (ls_uu - ls_lu)
            y_bounds(num) = i1 + 1.d0
            num = num+1
          endif
          if (ls_uu .lt. 0.d0) then
            x_bounds(num) = i0 + 1.d0
            y_bounds(num) = i1 + 1.d0
            num = num + 1
          endif
          if (ls_uu * ls_ul .lt. 0.d0) then
            x_bounds(num) = i0 + 1.d0
            y_bounds(num) = i1 - ls_ul / (ls_uu - ls_ul)
            num = num + 1
          endif
          ...
          if (ls_ul * ls_ll .lt. 0.d0) then
            x_bounds(num) = i0 - ls_ll / (ls_ul - ls_ll)
            y_bounds(num) = i1
            num = num + 1
          endif
