                                                 ...
                                                 const double time)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++)
    {
        ...
        for (CellIterator<NDIM> ci(patch->getBox()); ci; ci++)
        {
            ...
            double area = (*area_data)(idx);
            double cell_volume = dx[0] * dx[1] * (*vol_data)(idx);
            if (cell_volume > 0.0)
                if (!d_homogeneous_bdry) (*R_data)(idx) += pre_fac * d_k_off * area / cell_volume;
        }
    }
    Q_st_vec->close();
...
void
SurfaceBoundaryReactions::fillInitialCondition()
{
    EquationSystems* eq_sys = d_fe_data_manager->getEquationSystems();
...
void
SurfaceBoundaryReactions::beginTimestepping(const double /*current_time*/, const double /*new_time*/)
{
...
void
SurfaceBoundaryReactions::endTimestepping(const double /*current_time*/, const double /*new_time*/)
{
    EquationSystems* eq_sys = d_fe_data_manager->getEquationSystems();
    TransientExplicitSystem& q_sys = eq_sys->get_system<TransientExplicitSystem>(s_surface_sys_name);
    q_sys.update();
