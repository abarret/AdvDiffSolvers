                            ...
                            Pointer<PatchLevel<NDIM>> /*level*/)
{
    ...
    else
    {
        ...
        Pointer<CellData<NDIM, double>> cc_scr_data = patch->getPatchData(d_ls_scr_idx);
        if (cc_data)
        {
            cc_data->copy(*cc_scr_data);
        }
        else if (nc_data)
        {
            PatchMathOps math_ops;
            math_ops.interp(nc_data, cc_scr_data, patch, false);
        }
    }
} // setDataOnPatch
                            ...
                            const double /*data_time*/)
{
    ...
    if (c_data)
    {
        for (CellIterator<NDIM> ci(box); ci; ci++)
        {
            ...
            (*c_data)(idx) = (x_pt - d_cent).norm() - d_R;
        }
    }
    else if (n_data)
    {
        for (NodeIterator<NDIM> ci(box); ci; ci++)
        {
            ...
            (*n_data)(idx) = (x_pt - d_cent).norm() - d_R;
        }
    }
    else
    {
