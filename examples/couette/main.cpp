...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<LSFromLevelSet> vol_out_fcn = new LSFromLevelSet("LSOut", patch_hierarchy);
        ...
        Pointer<CartGridFunction> Q_out_init = new QFcn("QInit", app_initializer->getComponentDatabase("QInitial_out"));
        ...
        time_integrator->useLevelSetForTagging(ls_out_var, input_db->getBool("USE_OUTSIDE_LS_FOR_TAGGING"));
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            if (dump_postproc_data && (iteration_num % dump_postproc_interval == 0 || last_step))
            {
                ...
                const int ls_in_n_idx =
                    var_db->mapVariableAndContextToIndex(ls_in_var, time_integrator->getCurrentContext());
                const int vol_in_idx = var_db->mapVariableAndContextToIndex(
                    time_integrator->getVolumeVariable(ls_in_var), time_integrator->getCurrentContext());
                const int area_in_idx = var_db->mapVariableAndContextToIndex(
                    time_integrator->getAreaVariable(ls_in_var), time_integrator->getCurrentContext());
                ...
                const int vol_out_idx = var_db->mapVariableAndContextToIndex(
                    time_integrator->getVolumeVariable(ls_out_var), time_integrator->getCurrentContext());
