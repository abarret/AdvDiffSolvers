...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                for (CellIterator<NDIM> ci(box); ci; ci++)
                {
                    ...
                    (*wgt_data)(idx) =
                        ls < -app_initializer->getComponentDatabase("PoissonSolver")->getDouble("eps") ? 1.0 : 0.0;
                }
            }
        }
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                for (CellIterator<NDIM> ci(box); ci; ci++)
                {
                    const CellIndex<NDIM>& idx = ci();
                    const double ls = ADS::node_to_cell(idx, *ls_data);
                    (*wgt_data)(idx) = ls < -app_initializer->getComponentDatabase("PoissonSolver")->getDouble("eps") ?
#if (NDIM == 2)
                                           dx[0] *
                                               dx[1]
#endif
...

        {
            ...
            double eps = app_initializer->getComponentDatabase("PoissonSolver")->getDouble("eps");
            // Mask the error
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                Pointer<Patch<NDIM>> patch = level->getPatch(p());
