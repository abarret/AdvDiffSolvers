...
void
fillSystem(EquationSystems* eq_sys, std::string sys_name, std::function<double(const IBTK::VectorNd& x)> fcn)
{
...
void
fillGhostCells(Vec& x,
               const std::shared_ptr<GhostPoints>& ghosts,
               const std::shared_ptr<GlobalIndexing>& indexing,
               std::function<double(const IBTK::VectorNd& x)> fcn)
{
    ...
    {
        ierr = VecSetValue(x, ghost_idx_map.at(ghost.getId()), fcn(ghost.getX()), INSERT_VALUES);
        IBTK_CHKERRQ(ierr);
    }
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        muParserCartGridFunction b_fcn("B", app_initializer->getComponentDatabase("B_db"), grid_geometry);
        muParserCartGridFunction x_fcn("X", app_initializer->getComponentDatabase("X_db"), grid_geometry);
...
#define DRAW_OUTPUT
        ...
        fillSystem(bdry_eq_sys, "b", b);
        fillSystem(bdry_eq_sys, "exact", X);
