...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        pout << "Mapping indices\n";
        GlobalIndexing indexing("GlobalIndexing", patch_hierarchy, fe_mesh_partitioner, "exact", nullptr, 0);
        indexing.setupDOFs();
        const int eul_map = indexing.getEulerianMap();
        const std::map<int, int> lag_map = indexing.getLagrangianMap();
        const std::vector<int>& dofs_per_proc = indexing.getDofsPerProc();
