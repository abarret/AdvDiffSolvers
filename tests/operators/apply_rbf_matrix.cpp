...
{
#if (NDIM == 2)
    return (c - 2.0 * d * M_PI * M_PI) * std::sin(M_PI * p(0)) * std::sin(M_PI * p(1));
#else
...
//#define DRAW_OUTPUT
        ...
        Vec b_vec;
        int ierr = VecDuplicate(x_vec, &b_vec);
        IBTK_CHKERRQ(ierr);
        ierr = MatMult(mat, x_vec, b_vec);
        IBTK_CHKERRQ(ierr);
        // Now copy data back to original specifications
        copyDataFromPetsc(b_vec, b_eul_vec, patch_hierarchy, b_bdry_sys, eul_map, lag_map, dofs_per_proc);
