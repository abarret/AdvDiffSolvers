...
#define DRAW_DATA 1
        ...
        sharp_interface::CCSharpInterfaceScheduledJacobiSolver srj_solver(
            "SharpInterfaceSolver", app_initializer->getComponentDatabase("SRJ_Solver"), ghost_fill, bdry_fcn);
        srj_solver.setHomogeneousBc(false);
        srj_solver.setSolutionTime(0.0);
        srj_solver.initializeSolverState(x_vec, y_vec);
        srj_solver.solveSystem(x_vec, y_vec);
